...
describe("NginxFile Model", () => {
  ...
  test("should verify relationships with Machine collection work correctly using publicId", async () => {
    ...
    const appMachine = await Machine.findOne({
      publicId: nginxFile.appHostServerMachinePublicId,
    });
    const nginxMachine = await Machine.findOne({
      publicId: nginxFile.nginxHostServerMachinePublicId,
    });
